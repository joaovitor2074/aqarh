...
const outputPath = path.join(__dirname, "../../data/resultado_linha_pesquisadores.json");
...
async function processPerson(mainPage, rowHandle, index, total) {
  const basePerson = await extractPersonFromRow(rowHandle);
  const label = basePerson.nome || `Pesquisador ${index + 1}`;
...
export default async function scrapeLinhas(externalBrowser = null, chunkOptions = null) {
  console.log("========================================");
  console.log("Iniciando scraping de linhas dos pesquisadores");
  console.log("========================================");
  ...
  try {
    ...
    const tableHandle = await findPeopleTable(mainPage, "pesquisador");
    const rows = sliceRows(await getValidRows(tableHandle), chunkOptions);

    console.log(`Pesquisadores para processar: ${rows.length}`);

    for (let i = 0; i < rows.length; i++) {
      const result = await processPerson(mainPage, rows[i], i, rows.length);
    ...
    console.log(`Concluido: ${resultados.length} pesquisadores, ${withData} com linhas, ${errors} erros`);
